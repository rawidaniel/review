$(document).ready(function() {
    ...
    input.addEventListener("input", async function(){
        ...
        $("#ser-loading").empty().html('<div class="spinner-grow text-light" role="status">\
        ...
        if ((searchStr).length == 0) {
            ...
            $(".restaurants").empty()
            let response =  await fetch('http://18.205.104.232:5001/api/v1/restaurants')
            .catch(error => {
            ...
            for (let i = 0; i < restaurantLists.length; i++) {
                if (serachRegex.test(restaurantLists[i].name)) {
                    var restaurant = '<a href="/foods?restaurant_id=' 
                    + restaurantLists[i].id + '"><section class="restaurant">\
                    <h2>' + restaurantLists[i].name + '&nbsp\
                    ...
                    <div><img class="img-restr" src="./static/images/uploads/' + restaurantLists[i].image + '"\
            ...
            if (html == "") {
                document.querySelector(".restaurants").innerHTML = `<h1 style="color: red">No results found for "${searchStr}"</h1>` 
            } else {
